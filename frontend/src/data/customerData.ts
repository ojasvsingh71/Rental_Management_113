...
export const generateProducts = (count: number): Product[] => {
  ...
  return Array.from({ length: count }, (_, i) => ({
    id: i + 1,
    name: `${baseProducts[i % baseProducts.length]} ${
      i > 11 ? "Pro v" + Math.floor(i / 12) : "Pro"
    }`,
    image: `https://images.pexels.com/photos/${162553 + i * 100}/pexels-photo-${
      162553 + i * 100
    ...
    features: ["Feature A", "Feature B", "Feature C", "Feature D"].slice(
      0,
      2 + (i % 3)
    ),
    location: [
...
export const generateRentals = (count: number): Rental[] => {
  ...
  return Array.from({ length: count }, (_, i) => ({
    ...
    product: `${products[i % products.length]} ${
      i > 4 ? "v" + Math.floor(i / 5) : ""
    }`,
    image: `https://images.pexels.com/photos/${162553 + i * 100}/pexels-photo-${
      162553 + i * 100
    }.jpeg?auto=compress&cs=tinysrgb&w=400`,
    status: statuses[i % statuses.length],
    pickup: new Date(Date.now() - i * 7 * 24 * 60 * 60 * 1000)
      .toISOString()
      .split("T")[0],
    return: new Date(Date.now() + (7 - i) * 24 * 60 * 60 * 1000)
      .toISOString()
      .split("T")[0],
...
export const generateContracts = (count: number): Contract[] => {
  return Array.from({ length: count }, (_, i) => ({
    ...
    signedDate: new Date(Date.now() - i * 7 * 24 * 60 * 60 * 1000)
      .toISOString()
      .split("T")[0],
    terms: [
...
export const generateCalendarEvents = (count: number): CalendarEvent[] => {
  ...
  return Array.from({ length: count }, (_, i) => ({
    ...
    title: `${
      types[i % types.length].charAt(0).toUpperCase() +
      types[i % types.length].slice(1)
    } - ${products[i % products.length]}`,
    date: new Date(Date.now() + (i - 15) * 24 * 60 * 60 * 1000)
      .toISOString()
      .split("T")[0],
